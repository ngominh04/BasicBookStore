...
@WebServlet("/order")
@MultipartConfig(fileSizeThreshold = 1021*1024*2,
...
public class OrderServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (OrderDao.checkAndUpdateAvaiableBookOfOrder(order)) {
			...
			if (insertResult) {// hoàn tất việc tạo đơn hàng
				request.setAttribute("cartOfCustomer", MyUtils.getCartOfCustomer(session));
				request.setAttribute("orderOfCustomer", order);
				MyUtils.deleteCart(session); // xóa giỏ hàng khỏi session
			...
			MyUtils.updateCartOfCustomer(session,
	...
	public void validateOrderForm(String deliveryAddress,String paymentMode,Part filePart,List<String> errors) {
	...
	private Order createOrder(String deliveryAddress,String paymentMode,Part filePart,HttpSession session)throws IOException {
		Order order = new Order();
		Date now = new Date();

		Cart cartOfCustomer = MyUtils.getCartOfCustomer(session);
		String customerUsername = MyUtils.getLoginedUser(session).getUsername();
		...
		if ("cash".equals(paymentMode)) {
			order.setOrderStatus((byte)2); // đã xác nhận và đang
																	// chuyển hàng
			...
			String fileName = customerUsername + "_" + MyUtils.getTimeLabel()
					+ MyUtils.extractFileExxtension(filePart);
			String appPath = getServletContext().getRealPath(""); // Thư mục gốc của ứng
																	// dụng Web
			filePart.write(MyUtils.getFolderUpload(appPath, "transfer-img-upload")
					.getAbsolutePath() + File.separator + fileName);
			order.setOrderStatus((byte)1); //chờ xác nhận chuyển khoản
